...
class res_partner(osv.osv):
    ...
    def _get_employee(self, cr, uid, ids, field_name, arg, context=None):
        ...
        for rec in self.browse(cr, uid, ids, context=context):
            employee_id=self.partner_has_employee(cr, uid, rec.id)
            if employee_id:
                res[rec.id] = employee_id
        
        return res

    _columns = {
        'employee_id': fields.function(_get_employee, type="many2one",relation="hr.employee", string='Employee'),
    }


    #creates employee and redirects to it
    def create_employee(self, cr, uid, partner_id, context):
        if not self.partner_has_employee(cr, uid, partner_id):
    ...
    def create_user_from_employee(self, cr, uid, employee_id=False):
        ...
        if employee_id:
            ...
            if res:
                ...
                #what shall be the user's login?
                user_exists=True
                while user_exists:        
                    if res.work_email:
